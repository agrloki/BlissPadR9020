...
const char *AudioHardware::inputPathNameDefault = "SRC2 Output";
const char *AudioHardware::inputPathNameCamcorder = "SRC2 Output";
const char *AudioHardware::inputPathNameVoiceRecognition = "SRC2 Output";
const char *AudioHardware::inputPathNameVoiceCommunication = "SRC2 Output";
...
void AudioHardware::closeInputStream(AudioStreamIn* in) {
    ...
    spIn.clear();
ALOGV("AudioHardware::clear spIN");
}
    ...
    if (status == NO_ERROR) {
        ...
        if (mMode == AudioSystem::MODE_NORMAL && mInCallAudioMode) {
            ...
            if (mMixer != NULL) {
                ...
                struct mixer_ctl *ctl= mixer_get_ctl_by_name(mMixer, "MODE_SEL");
                TRACE_DRIVER_OUT
                if (ctl != NULL) {
                    ALOGV("setMode() reset Playback Path to RT5625_CAPTURE_NORMAL");
                    TRACE_DRIVER_IN(DRV_MIXER_SEL)
                    mixer_ctl_set_enum_by_string(ctl, "RT5625_CAPTURE_NORMAL");
                    TRACE_DRIVER_OUT
                }
            }
...
const char *AudioHardware::getOutputRouteFromDevice(uint32_t device)
{
    switch (device) {
    case AudioSystem::DEVICE_OUT_EARPIECE:
        return "RT5625_HEADPHONE_NORMAL";
    case AudioSystem::DEVICE_OUT_SPEAKER:
        if (mMode == AudioSystem::MODE_RINGTONE) return "RT5625_SPEAKER_RINGTONE";
        else return "RT5625_SPEAKER_NORMAL";
    case AudioSystem::DEVICE_OUT_WIRED_HEADPHONE:
        if (mMode == AudioSystem::MODE_RINGTONE) return "RT5625_HEADPHONE_RINGTONE";
        else return "RT5625_HEADPHONE_NORMAL";
    case AudioSystem::DEVICE_OUT_WIRED_HEADSET:
        if (mMode == AudioSystem::MODE_RINGTONE) return "RT5625_HEADSET_RINGTONE";
        else return "RT5625_HEADSET_NORMAL";
    ...
    case AudioSystem::DEVICE_OUT_BLUETOOTH_SCO_HEADSET:
    case AudioSystem::DEVICE_OUT_BLUETOOTH_SCO_CARKIT:
        return "RT5625_BLUETOOTH_INCALL";
    default:
        return "disable";
    }
}
...
const char *AudioHardware::getInputRouteFromDevice(uint32_t device)
{
    ...
    ALOGV("RT5625_CAPTURE_NORMAL");
        return "RT5625_CAPTURE_NORMAL";
    case AudioSystem::DEVICE_IN_WIRED_HEADSET:
        return "RT5625_HEADSET_INCALL";
...
status_t AudioHardware::setInputSource_l(audio_source source)
{
     ...
     if (source != mInputSource) {
         ALOGV("Start select InputSource");

           if ((source == AUDIO_SOURCE_DEFAULT) || (mMode != AudioSystem::MODE_IN_CALL)) {
             if (mMixer) {
                 TRACE_DRIVER_IN(DRV_MIXER_GET)
                 struct mixer_ctl *ctl= mixer_get_ctl_by_name(mMixer, "IIS src select Mux");
                 TRACE_DRIVER_OUT
                 if (ctl == NULL) {
                     return NO_INIT;
                 }
			 ...
			 ALOGV("setInputSource_l(%d)", sourceName);
...
status_t AudioHardware::AudioStreamOutALSA::open_l()
{
    ...
    if (mMixer) {
        ...
        TRACE_DRIVER_IN(DRV_MIXER_GET)
        mRouteCtl = mixer_get_ctl_by_name(mMixer, "MODE_SEL");
        TRACE_DRIVER_OUT
    }
